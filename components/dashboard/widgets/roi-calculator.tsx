...
export function ROICalculator({ roiCalculation, setRoiCalculation, calculateROI }: ROICalculatorProps) {
  return (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setRoiCalculation((prev) => ({ ...prev, conversionRate: Number(e.target.value) }))}
              className="mt-1 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
          </div>
          <Button
          ...
          {roiCalculation.result !== 0 && (
            <div className="mt-4 p-4 bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 rounded-lg">
              ...
              <div className="space-y-1 text-xs md:text-sm dark:text-gray-300">
                <p>Expected Conversions: {Math.round((roiCalculation.reach * roiCalculation.conversionRate) / 100)}</p>
                <p>
                  Projected Revenue: $
                  {Math.round((roiCalculation.reach * roiCalculation.conversionRate * 50) / 100).toLocaleString()}
                </p>
                <p
