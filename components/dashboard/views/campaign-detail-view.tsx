...
export function CampaignDetailView({
  ...
  return (
    <div className="space-y-6 p-1 sm:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="space-y-2 text-sm">
            ...
            <p className="dark:text-gray-300">
              <strong>Duration:</strong> {campaign.startDate} -{" "}
              {campaign.endDate}
            </p>
          </div>
        </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="dark:text-gray-300">
                    {Math.round(
                      (campaign.actualReach / campaign.expectedReach) * 100
                    )}
                    %
                  </span>
                </div>
                <Progress
                  value={(campaign.actualReach / campaign.expectedReach) * 100}
                />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
      ...
      <div>
        <h3 className="font-semibold mb-3 dark:text-white">
          Assigned KOLs ({campaignKOLs.length})
        </h3>
        <div className="space-y-3">
          {campaignKOLs.map((kol) => (
            <div
              key={kol.id}
              className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between p-3 border rounded-lg dark:border-gray-700"
            >
